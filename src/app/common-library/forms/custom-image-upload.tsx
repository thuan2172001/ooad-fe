...
function CustomImageUpload({
  ...
  const _secondaryLabel = useMemo(() => (secondaryLabel ? _.isString(secondaryLabel) ? intl.formatMessage({ id: secondaryLabel }) : secondaryLabel : null), []);
  ...
  return (
    <div className={mode === 'horizontal' ? 'row' : ''}>
      ...
      {_secondaryLabel && (
        <div className={mode === 'horizontal' ? GetClassName(labelWidth, true) : ''}>
          <label className={mode === 'horizontal' ? 'mb-0 mt-2' : ''}>
            {_secondaryLabel}{required && <span className="text-danger">*</span>}
          </label>
        </div>
      )}
